(function(root, factory) {
  ...
  function calculatePricing(input = {}) {
    ...
    const toNumber = (id, fallback = 0) => {
      ...
      if (!Number.isInteger(n) || n < 0 || n > 10000) throw new Error('Enter a whole quantity from 0 to 10000 for ' + id + '.');
      return n;
    };
    ...
    services.forEach(item => { if (seen.has(item.serviceId)) throw new Error('Duplicate subscription service.'); seen.add(item.serviceId); });
  function propertyAdjustments(type) {
    ...
    if (type === 'house' || type === 'townhouse') {
      ...
      if (yes('housePool') && !checked('removePoolService') && !selectedServices().some(item => item.serviceId === 'pool-cleaning')) {
  ...
  function serviceAdjustments(type, selected) {
    ...
    selected.forEach((item) => {
      ...
      firstClean += includedFrequency ? 0 : catalogue.firstAdd;
      recurring += Math.max(0, catalogue.monthly[item.frequency] - includedMonthly);
    });
  ...
  function conditionAdjustments() {
    ...
    if (condition === 'standard') return { firstClean: 0, recurring: 0, complexity: 1 };
  ...
  function calculate() {
    ...
    return {
      ...
      annualRecurring,
      worker,
      gstNote: '10% subscription discount applied once; GST added after discount. No payment surcharge.'
    };
  }
