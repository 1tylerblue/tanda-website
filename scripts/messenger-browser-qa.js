async (page) => {
  ...
  await page.route('https://tanda-pro-cleaning-api.onrender.com/api/analytics-config', (route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({
      enabled: true,
      projectKey: 'phc_abcdefghijklmnopqrstuvwxyz123456',
      host: 'https://eu.i.posthog.com',
      uiHost: 'https://eu.posthog.com',
      environment: 'test',
      release: 'messenger-qa',
  ...
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.goto(`${baseUrl}/index.html`, { waitUntil: 'domcontentloaded' });
  await page.waitForFunction(() => Array.isArray(window.posthog?._i) && window.posthog._i.length > 0, null, { timeout: 2000 }).catch(() => {});
  await page.evaluate(() => {
    window.__messengerEvents = [];
    const capture = (eventName, properties) => window.__messengerEvents.push({ eventName, properties });
    window.posthog = window.posthog || [];
    window.posthog.capture = capture;
    const initPayload = Array.isArray(window.posthog._i) ? window.posthog._i[0] : null;
    const config = initPayload && initPayload[1];
    if (config && typeof config.loaded === 'function') {
      config.loaded({
        capture,
